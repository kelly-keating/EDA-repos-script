...
# Enter your username here (will then request password)
GITHUB_USER="kelly-keating"
...
function make_new_repo {
    ...
    # curl at github api
    ( curl --user $GITHUB_USER -X POST --data "$new_repo_data" https://api.github.com/orgs/$COHORT/repos > /dev/null ) && ( echo "\nSuccessfully created repo $repo" )
}
...
function protect_master {
    ...
    ( curl --user $GITHUB_USER -X PUT --data "$permissions" https://api.github.com/repos/$COHORT/$repo/branches/master/protection > /dev/null ) && ( echo "\nSuccessfully updated master permissions on $repo" )
}
